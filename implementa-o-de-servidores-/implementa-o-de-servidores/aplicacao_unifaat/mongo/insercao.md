...
db.client_process.insertMany([{"client_id": "id_do_cliente_maria_silva",
...
db.events.insertMany ([{ "client_id": "id_do_cliente_maria_silva",
  "enterprise": null,
  "cnpj_enterprise": null,
  "freight": 50.00,
...
2.4 Clientes corporativos (somente full_name e cnpj_enterprise)
db.client.find(
  { enterprise: { $ne: null } },
  { full_name: 1, cnpj_enterprise: 1, _id: 0 }
)

2.5 Processos de cobrança, ordenados por valor decrescente
db.client_processes.find({ class: "Cobrança" }).sort({ value: -1 })

*Exercício 3: Atualizando Dados

3.1 Atualizar Status do Processo
Altere o status do processo com o número PROC-2023-001 para "concluído".
...
Adicione o campo note_doc com o valor "OBS-MS-001.txt" ao evento de Maria Silva.
db.events.updateOne(
  { client_id: ObjectId("ID_DA_MARIA_SILVA") },
  { $set: { note_doc: "OBS-MS-001.txt" } }
)

3.3 Incrementar Quantidade de Limpezas
Aumente em 1 a amount_of_cleaning para o evento da "Empresa Soluções Ltda".
db.events.updateOne(
  { client_id: ObjectId("ID_DA_EMPRESA_SOLUCOES") },
...
Remova o processo com o número PROC-2023-002.
db.client_processes.deleteOne(
  { number: "PROC-2023-002" }
)

4.2 Remover Clientes sem CNPJ
Remova todos os clientes onde o campo cnpj_enterprise seja null.
...
5.1 Índice no Nome do Cliente
Crie um índice no campo full_name da coleção client.
db.client.createIndex({ full_name: 1 })

5.2 Visualizar Índices
Liste todos os índices da coleção client.
